# AGENTS.md - Global Project Guidelines
...
- 作業で得た知見・ユーザーの好み・プロジェクト固有の事情は `z-ai/` ではなく
  Codex ビルトインの memory に保存する。`z-ai/` は計画書と進捗の置き場に限る。
...
## Database Migration（DBスキーマ変更の絶対ルール）

DBスキーマ変更は本番障害・データ消失の最大リスク領域。以下の7つを必ず守る。

1. **DBスキーマ変更は必ず migration SQL として作る**（手動 ALTER 禁止）
2. **migration ファイルは Git 管理する**（マージ後は編集・削除しない）
3. **local → staging → production の順に適用する**
4. **production に直接 SQL を流さない**（migration ツール経由のみ）
5. **破壊的変更は原則避ける**（`DROP COLUMN`、rename、非互換型変更は段階的デプロイで）
6. **rollback 前提ではなく forward-only で設計する**（戻すなら新しい migration で打ち消す）
7. **本番適用前に backup / export / staging 検証を必ず行う**

- 詳細は `rules/core/database-migration.md` を参照
- 実作業は `/database-migration` スキルで適用ルール・テンプレート・破壊的変更プレイブックを呼び出せる
...
詳細は `rules/core/Codex-usage.md` を参照。
...
worktree作成時に `.Codex/` ディレクトリは自動共有される（gitで管理されているため）。
