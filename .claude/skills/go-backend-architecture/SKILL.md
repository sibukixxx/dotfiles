...
参照ルール: `rules/core/go-backend-architecture.md`
...
- **Go ライブラリ / SDK** （`go get` で他サービスから利用される `*.go` 公開パッケージ）
  → 規約は適用しない。`rules/core/go-backend-architecture.md` の「Go ライブラリのレイアウト」を参照
- フロントエンド / 非 Go プロジェクト
...
- 適用ルール: rules/core/go-backend-architecture.md
...
- `rules/core/go-backend-architecture.md` - 本スキルが参照する規約
- `rules/core/database-migration.md` - migration の運用ルール
- `rules/core/tdd.md`, `rules/core/testing.md` - テスト規約
- `rules/core/ai-agent-o11y.md` - AI エージェント計装
