...
4. **既定の出力スタイルと重複していないか確認**。Claude Code は既定で簡潔な出力スタイルを持ち、CLAUDE.md に簡潔ルールを1行足すだけでも代替できる。Caveman・Claude Token Efficient はこれと実質重複する。
5. **外部ツールの前に運用習慣を直す**。Effort Level の max 常用と Subagent の反射呼び（4-7x 消費）は、ツール導入なしで直せる最大の浪費源。`rules/core/claude-code-usage.md` を先に確認する。
...
→ **Claude Code は既定で簡潔出力スタイルを持つことを伝える**（CLAUDE.md に簡潔ルールを足すだけでも代替可能）。
...
- ベンダーの主張削減率をそのまま提示する
- 既定の出力スタイルと重複するスタイル系ツール（Caveman / Claude Token Efficient）を最初に勧める
- 計測なしの導入を勧める
