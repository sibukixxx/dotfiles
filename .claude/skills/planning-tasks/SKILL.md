---
name: planning-tasks
description: 承認済みの設計書（DESIGN.md）からTDD準拠のTODO.mdを作成します。analyzing-requirementsスキルで設計が完了・承認された後に使用します。/impl コマンドで実装できる形式のタスクリストを生成します。
---
...
承認済みの設計書（DESIGN.md）を読み込み、TDD（テスト駆動開発）に準拠したTODO.mdを作成する。生成されるTODO.mdは/impl コマンドで直接使用できる形式となる。
  ...
  questions: [
    {
      ...
      header: "実装開始",
      options: [
        { label: "開始する", description: "/impl コマンドで実装を開始" },
