...
AskUserQuestionツールで検出された問題をユーザーに提示し、修正対象を選択してもらう：

```javascript
AskUserQuestion({
  questions: [
    {
      question: "以下の問題が検出されました。修正する項目を選択してください。",
      header: "修正対象",
      options: [
        // 検出された問題を動的にリスト化
        // 例:
        { label: "[Critical] ハードコードされたAPIキー", description: "src/auth.rs:45 - セキュリティ上の問題" },
        { label: "[Warning] 関数が長すぎる", description: "src/user.rs:120 - 60行を超えている（分割推奨）" },
